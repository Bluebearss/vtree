...
/* FUNCTION PROTOTYPES ADDED MYSELF */
...
int	user_file_list_supplied = 0;
	...
	static struct option long_options[] = {
	...
	#ifdef LINUX
        while ((option = getopt_long(argc, argv, "dfh:iostqvVl", long_options, 0)) != EOF) {
    #else
		...
		if (err) {
			fprintf(stderr,"%s: [ -d ] [ -f ] [ -h # ] [ -i ]",Program);
			#ifdef MEMORY_BASED
			fprintf(stderr," [ -o ]");
			#endif
			fprintf(stderr," [ -s ] [ -t ] [ -q ] [ -v ] [ -V ] ");
			#ifdef LSTAT
			fprintf(stderr,"[ -l ]\n");
			#else
			fprintf(stderr,"\n");
			...
			#ifdef LSTAT
			fprintf(stderr,"	-l	not to follow symbolic links\n");
			#endif
			exit(-1);
		}
